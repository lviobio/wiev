...
interface ModelManagerContract
{
    ...
    /**
     * Колбэк, выполняемый внутри транзакции flush(), после записи графа и перед
     * коммитом — для проверок и записей, которые должны откатиться вместе со всем
     * остальным. Исключение из колбэка откатывает транзакцию.
     */
