import { propsSchemaKey } from '@/core/windows/withPropsSchema'
...
export function useDesktopWindows() {
  ...
  function close(key: number) {
    const idx = state.windows.findIndex((w) => w.key === key)
    if (idx !== -1) state.windows.splice(idx, 1)
  }

  function bringToFront(key: number) {
    const w = state.windows.find((w) => w.key === key)
    if (!w) return
    w.z = ++state.nextZ
  }

  function updateX(key: number, x: number) {
    const w = state.windows.find((w) => w.key === key)
    if (!w) return
    w.x = x
  }

  function updateY(key: number, y: number) {
    const w = state.windows.find((w) => w.key === key)
    if (!w) return
    w.y = y
  }
